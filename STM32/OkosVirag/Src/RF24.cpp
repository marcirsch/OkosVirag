...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len) {
	...
	*ptx++ = ( R_REGISTER | ( REGISTER_MASK & reg ) );
	...
	SPI_Tansfernb( (char *) spi_txbuff, (char *) spi_rxbuff, size);
	...
	beginTransaction();
	status = SPI_Tansfer( R_REGISTER | ( REGISTER_MASK & reg));
	while (len--) {
		*buf++ = SPI_Tansfer(0xff);
	}
	endTransaction();
...
uint8_t RF24::read_register(uint8_t reg) {
	...
	*ptx++ = RF24_NOP; // Dummy operation, just for reading
	...
	SPI_Tansfernb( (char *) spi_txbuff, (char *) spi_rxbuff, 2);
	...
	beginTransaction();
	SPI_Tansfer( R_REGISTER | ( REGISTER_MASK & reg));
	result = SPI_Tansfer(0xff);
	endTransaction();
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len) {
	...
	*ptx++ = *buf++;
	...
	beginTransaction();
	status = SPI_Tansfer( W_REGISTER | ( REGISTER_MASK & reg));
	while (len--)
		SPI_Tansfer(*buf++);
	endTransaction();
...
uint8_t RF24::read_payload(void* buf, uint8_t data_len) {
	...
	*ptx++ = RF24_NOP;
	...
	beginTransaction();
	status = SPI_Tansfer( R_RX_PAYLOAD);
	while (data_len--) {
		*current++ = SPI_Tansfer(0xFF);
	}
	while (blank_len--) {
		SPI_Tansfer(0xff);
	}
...
RF24::RF24(uint16_t _cepin, uint16_t _cspin) :
		ce_pin(_cepin), csn_pin(_cspin), p_variant(false), payload_size(32), dynamic_payloads_enabled(
				false), addr_width(5), csDelay(5)  //,pipe0_reading_address(0)
{
...
write_register(DYNPD,
		read_register(
		DYNPD) | _BV(DPL_P5) | _BV(DPL_P4) | _BV(DPL_P3) | _BV(DPL_P2)
...
SPI_Tansfern( (char *) spi_txbuff, size);
endTransaction();
#else
beginTransaction();
SPI_Tansfer(W_ACK_PAYLOAD | (pipe & 0x07));
